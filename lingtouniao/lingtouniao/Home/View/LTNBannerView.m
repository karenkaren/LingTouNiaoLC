...
@interface LTNBannerView ()<UIScrollViewDelegate>
...
- (void)setupUI
{
    ...
    if (count == 1) {
        LTNBanner * banner = self.bannersList.firstObject;
...
- (void)tap:(UIGestureRecognizer *)recognizer
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(bannerView:banner:)]) {
        ...
        LTNBanner * banner = self.bannersList[imageView.tag];
...
//    page = page >= self.bannersList.count ? 0 : page ;
...
//     NSLog(@"%.2f", scrollView.contentOffset.x);
    int page = (self.scroll.contentOffset.x + kContentOffset) / kWidth;
    page = (page == (self.bannersList.count + 1) ? 0 : page - 1);
    self.page.currentPage = page;
    
//    if (self.startDrag) {
//        self.startDrag = NO;
//        if (scrollView.contentOffset.x >= (self.bannersList.count - 1) * self.width * kRatio + kContentOffset) {
//            self.startDrag = NO;
//            [self.scroll setContentOffset:CGPointMake(scrollView.contentOffset.x - ((self.bannersList.count - 1) * self.width * kRatio + kContentOffset), 0)];
//            [UIView animateWithDuration:0.5f animations:^{
//                [self.scroll setContentOffset:CGPointMake(kContentOffset, 0)];
//            }];
//        }
//        
//        if (scrollView.contentOffset.x <= 2 * self.width * kRatio - self.width) {
//            self.startDrag = NO;
//            CGFloat offsetX = self.bannersList.count * self.width * kRatio + scrollView.contentOffset.x;
//            [self.scroll setContentOffset:CGPointMake(offsetX, 0)];
//            [UIView animateWithDuration:0.5f animations:^{
//                [self.scroll setContentOffset:CGPointMake((self.bannersList.count - 2) * self.width * kRatio + kContentOffset, 0)];
//            }];
//        }
//    }
}
...
//{
//    int currentPage = (self.scroll.contentOffset.x + kContentOffset) / kWidth;
//    [self updateScrollContentOffsetWithPage:currentPage];
//}
//
//- (void)updateScrollContentOffsetWithPage:(NSInteger)currentPage
//{
//    if (currentPage == 0) {
//        // 序号0，循环到最后1页
//        [self.scroll setContentOffset:CGPointMake(kContentOffset, 0) animated:YES];
//        //        [self.scroll scrollRectToVisible:CGRectMake(kContentOffset, 0, kWidth, kHeight) animated:YES];
//    } else if (currentPage == (self.bannersList.count + 1)) {
//        // 最后+1，循环到第1页
//        [self.scroll setContentOffset:CGPointMake(self.width * kRatio * (self.bannersList.count - 1) + kContentOffset, 0) animated:YES];
//        //        [self.scroll scrollRectToVisible:CGRectMake(self.width * kRatio * (self.bannersList.count - 1) + kContentOffset, 0, kWidth, kHeight) animated:YES];
//    }else {
//        [self.scroll setContentOffset:CGPointMake(self.width * kRatio * (currentPage - 1) + kContentOffset, 0) animated:YES];
//        //        [self.scroll scrollRectToVisible:CGRectMake(self.width * kRatio * (currentPage - 1) + kContentOffset, 0, kWidth, kHeight) animated:YES];
//    }
//
//}
