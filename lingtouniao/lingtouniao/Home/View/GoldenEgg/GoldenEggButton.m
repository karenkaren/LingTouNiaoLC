...
//    self.bounds = CGRectMake(0, 0, eggLabel.width + eggImageView.width - 10, height);
    ...
    [self addSubview:eggImageView];
    self.egg = eggImageView;
    CGFloat width = MAX(kGeneralHeight, eggImageView.width);
    CGFloat height = MAX(kGeneralHeight, eggImageView.height);
    self.bounds = CGRectMake(0, 0, width, height);
    eggImageView.center = CGPointMake(width * 0.5, height * 0.5);
    
    // 圆背景
    self.backgroundColor = [UIColor whiteColor];
    self.layer.cornerRadius = MIN(width, height) * 0.5;
    self.layer.masksToBounds = YES;
    self.layer.borderColor = [HexRGB(0xe2e2e2) CGColor];
