...
//            self.popupView = [[PopupView alloc] initWithPrompt:locationString(@"product_detail_dialog1") attributes:attributes inView:self.view fromRect:self.footerView.frame];
        } else if (self.product.productRemainAmount && self.product.productRemainAmount <= 3 * self.product.staInvestAmount) {
            // 尾单
...
- (void)refreshFooterView
{
    ...
    self.startAmount = MIN(self.product.staInvestAmount, self.product.productRemainAmount);
...
- (void)purchseProduct:(UIButton *)button
{
    ...
    if ([self.product.productType isEqualToString:@"XSB"] && self.product.productDeadline <= 10 && purchaseAmount > self.startAmount - 0.001) {
...
- (void)purchaseOnce:(UIButton *)button
{
    ...
    // 一次性全投金额
    double purchaseOnceAmount = usableAmount < self.startAmount ? self.startAmount : floor(MIN(usableAmount, self.product.productRemainAmount));
