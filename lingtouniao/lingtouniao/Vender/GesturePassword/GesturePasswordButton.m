...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        success=YES;
        [self setBackgroundColor:HexRGB(0xFEFCF5)];
    }
    return self;
}
...
- (void)drawRect:(CGRect)rect
{
   ...
   // CGContextRef context = UIGraphicsGetCurrentContext();
    ...
    if (selected) {
        image=[UIImage imageNamed:@"gesture_node_selected"];
    }
    else{
        image=[UIImage imageNamed:@"gesture_node_normal"];
    }
    ...
    if (selected) {
        ...
        CGContextSetRGBStrokeColor(context, 0,1,1,1);//线条颜色 圆圈颜色
    }
