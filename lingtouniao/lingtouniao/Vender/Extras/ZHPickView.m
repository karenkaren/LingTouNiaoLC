...
-(instancetype)initPickviewWithArray:(NSArray *)array isHaveNavControler:(BOOL)isHaveNavControler  andDesArray:(NSMutableArray *)desArray andTextField:(UITextField *)textField{
    ...
    if (self) {
        ...
        self.desArray = [NSMutableArray arrayWithArray:desArray];
        self.selectBankField = textField;
...
-(UIToolbar *)setToolbarStyle{
    ...
    UIBarButtonItem *unitem=[[UIBarButtonItem alloc] initWithTitle:@" " style:UIBarButtonItemStylePlain target:nil action:nil];
    UIBarButtonItem *lefttem=[[UIBarButtonItem alloc] initWithTitle:[NSString stringWithFormat:@" %@", locationString(@"cancel")] style:UIBarButtonItemStylePlain target:self action:@selector(remove)];
    UIBarButtonItem *centerSpace=[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:self action:nil];
    UILabel *toolBarItemLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 180, 30)];
    [toolBarItemLabel setTextAlignment:NSTextAlignmentCenter];
    [toolBarItemLabel setBackgroundColor:[UIColor clearColor]];
    toolBarItemLabel.text = locationString(@"choose_bank");
    UIBarButtonItem *buttonLabel = [[UIBarButtonItem alloc] initWithCustomView:toolBarItemLabel];
    UIBarButtonItem *right=[[UIBarButtonItem alloc] initWithTitle:[NSString stringWithFormat:@"%@ ", locationString(@"btn_confirm")] style:UIBarButtonItemStylePlain target:self action:@selector(doneClick)];
    toolbar.items=@[unitem,lefttem,centerSpace,buttonLabel,centerSpace,right,unitem];
    return toolbar;
}
...
{
    UILabel *myLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 0.0, kScreenWidth, 30)];
    myLabel.textAlignment = NSTextAlignmentCenter;
    NSString *titleString =[NSString stringWithFormat:@"%@%@",_plistArray[row],_desArray[row]];
    NSMutableAttributedString * aAttributedString = [[NSMutableAttributedString alloc] initWithString:titleString];
    NSRange range= [titleString rangeOfString:_desArray[row]];
    [aAttributedString addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:12] range:range];
    myLabel.attributedText = aAttributedString;
...
{
    return [_plistArray objectAtIndex:row];
}
...
-(void)remove{
    if ([self.delegate respondsToSelector:@selector(changeViewFrame)]) {
        [self.delegate changeViewFrame];
    }
    [_selectBankField resignFirstResponder];
