...
+ (UIBarButtonItem*)barItemWithTitle:(NSString*)title image:(UIImage *)image highlightImage:(UIImage *)highlightImage style:(UIBarButtonItemStyle)style target:(id)target action:(SEL)action andColor:(UIColor*)color andIsRight:(NSInteger)isflag
{
    ...
    {
        [button setTitleColor:kNavigationTintColor forState:UIControlStateNormal];
    }
    
    button.titleLabel.font = [CustomerizedFont boldSystemFontOfSize:16.0f];
...
+ (UIBarButtonItem*)barItemWithTitle:(NSString*)title image:(UIImage *)image highlightImage:(UIImage *)highlightImage style:(UIBarButtonItemStyle)style target:(id)target action:(SEL)action space:(CGFloat)space
{
    ...
    UILabel *label = [Utility createLabel:[CustomerizedFont systemFontOfSize:16.0f] color:[UIColor blackColor]];
    ...
    buttonView.size = CGSizeMake(10 + imageView.width + space + label.width, MIN(SYSTEM_NAVIGATION_HEIGHT, MAX(imageView.height, label.height)));
    imageView.centerY = label.centerY = buttonView.myCenterY;
    imageView.left = 10;
