...
@interface LTNCrowdRecordController ()

@property (nonatomic) NSInteger total;

@end

@implementation LTNCrowdRecordController
...
- (void)getServiceData:(BOOL)isGetMore {
    ...
    [self apiForPath:kSearchOrderZc method:kGetMethod parameter:dic responseModelClass:nil onComplete:^(id response, id data, NSError *error) {
        if (!error && [data[@"orderZcList"] count] > 0) {
            weakSelf.total = [[data valueForKey:@"totalCount"] integerValue];
            [weakSelf.data addObjectsFromArray:[data valueForKey:@"orderZcList"]];
        }
    }];
}
