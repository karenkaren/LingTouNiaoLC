...
- (void)sendSingleEventWithParameters:(NSDictionary*)parameters
  ...
  if (self.userAgent) {
    ...
    [self GET:self.piwikPath parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull operation, id  _Nonnull responseObject) {
        successBlock();
...
- (void)sendBulkEventWithParameters:(NSDictionary*)parameters
  ...
  if (self.userAgent) {
    ...
    [self POST:self.piwikPath parameters:parameters progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        successBlock();
    } failure:^(NSURLSessionDataTask *task, NSError *error) {
        failureBlock([self shouldAbortdispatchForNetworkError:error]);
    }];
