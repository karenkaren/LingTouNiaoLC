...
static PiwikTracker *_sharedInstance1;
...
+ (instancetype)sharedInstanceWithSiteID1:(NSString*)siteID baseURL:(NSURL*)baseURL {
    ...
    return [self sharedInstanceWithSiteID1:siteID dispatcher:[self defaultDispatcherWithPiwikURL:baseURL]];
}
...
+ (instancetype)sharedInstanceWithSiteID1:(NSString*)siteID dispatcher:(id<PiwikDispatcher>)dispatcher {
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedInstance1 = [[PiwikTracker alloc] initWithSiteID:siteID dispatcher:dispatcher];
    });
    
    return _sharedInstance1;
}
...
- (NSString*)generatePageURL:(NSArray*)components {
  ...
  if (components) {
    NSString *pageURL = [NSString stringWithFormat:@"http://%@/%@", @"ios.lingtouniao.com", [components componentsJoinedByString:@"/"]];
    self.lastGeneratedPageURL = pageURL;
    return pageURL;
  } else if (self.lastGeneratedPageURL) {
    return self.lastGeneratedPageURL;
  } else {
    return [NSString stringWithFormat:@"http://%@", @"ios.lingtouniao.com"];
  }
